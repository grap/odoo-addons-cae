...
class TestModule(TransactionCase):
    ...
    def setUp(self):
        ...
        self.saleable_service_categ = self.env.ref(
            "product.product_category_3")
        self.expense_field = self.env.ref(
            "account."
            "field_product_category__property_account_expense_categ_id")
        self.income_field = self.env.ref(
            "account."
            "field_product_category__property_account_income_categ_id")
    ...
    def test_01_propagate_recursively(self):
        ...
        self._create_account_for_all_companies(
            "707TEST", "Sale", self.income_type.id)
        self.saleable_categ.write({
            "global_property_account_income_categ": "707TEST",
        })

        companies = self.ResCompany.with_context(
            active_test=False).search([
                ("fiscal_type", "in", [
                    "normal", "fiscal_mother", "fiscal_child"])])
    ...
    def test_02_fiscal_child_company(self):
        ...
        self.assertEqual(
            1, len(properties),
            "Create a new fiscal company should create properties")
